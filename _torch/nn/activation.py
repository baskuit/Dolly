...
from .functional import multi_head_attention_forward, dot_scaled_linear_forward
...
class MultiheadAttention(Module):
    ...
    def __init__(
        ...
        self.outer_dim = outer_dim if outer_dim is not None else embed_dim
        ...
        if self._qkv_same_embed_dim is False:
            self.q_proj_weight = Parameter(
                torch.empty((inner_dim, embed_dim), **factory_kwargs)
            )
            self.k_proj_weight = Parameter(
                torch.empty((inner_dim, self.kdim), **factory_kwargs)
            )
            self.v_proj_weight = Parameter(
                torch.empty((outer_dim, self.vdim), **factory_kwargs)
            )
        ...
        if bias:
            self.in_proj_bias = Parameter(
                torch.empty(2 * inner_dim + outer_dim, **factory_kwargs)
            )
        else:
            self.register_parameter("in_proj_bias", None)
        self.out_proj = NonDynamicallyQuantizableLinear(
            outer_dim, self.output_dim, bias=bias, **factory_kwargs
        )

        if add_bias_kv:
            self.bias_k = Parameter(torch.empty((1, 1, inner_dim), **factory_kwargs))
            self.bias_v = Parameter(torch.empty((1, 1, outer_dim), **factory_kwargs))
        else:
    ...
    def forward(
        ...
        if not self._qkv_same_embed_dim:
            attn_output, attn_output_logits = multi_head_attention_forward(
                query,
                key,
                value,
                self.embed_dim,
        ...
        if self.batch_first and is_batched:
            return attn_output.transpose(1, 0), attn_output_logits
        else:
            return attn_output, attn_output_logits


class DotScaledLinear(Module):
    ...
    def __init__(
        ...
        factory_kwargs = {"device": device, "dtype": dtype}
        super(DotScaledLinear, self).__init__()
        self.row_dim = row_dim
        self.dot_dim = dot_dim if dot_dim is not None else row_dim
        self.output_dim = output_dim if output_dim is not None else row_dim
        self._qkv_same_embed_dim = False

        self.q_proj_weight = Parameter(
            torch.empty((dot_dim, row_dim), **factory_kwargs)
        )
        self.k_proj_weight = Parameter(
            torch.empty((dot_dim, col_dim), **factory_kwargs)
        )
        self.v_proj_weight = Parameter(
            torch.empty((output_dim, col_dim), **factory_kwargs)
        )
    ...
    def forward(
        ...
        attn_output, attn_output_matrix = dot_scaled_linear_forward(
            row_words,
            col_words,
            col_words,
            q_proj_weight=self.q_proj_weight,
        ...
        return attn_output, attn_output_matrix
