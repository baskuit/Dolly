...
class AbstractBattle(ABC):
    ...
    def _check_damage_message_for_item(self, split_message: List[str]):
        ...
        if (
            ...
            self.get_pokemon(pkmn)._public_item = to_id_str(item)
    ...
    def _check_heal_message_for_item(self, split_message: List[str]):
        ...
        if len(split_message) == 5 and split_message[4].startswith("[from] item:"):
            ...
            if pkmn_object.item is not None:
                ...
                pkmn_object._public_item = to_id_str(item)
    ...
    def _parse_message(self, split_message: List[str]) -> None:
        ...
        if split_message[1] in self.MESSAGES_TO_IGNORE:
            ...
            elif split_message[5] in ["[from]Sleep Talk", "[from]Snatch"]:
                pokemon, move, presumed_target, sleep_talk = split_message[2:6]
            else:
                pokemon, move, presumed_target = split_message[2:5]
                self.logger.warning(
            ...
            if weather == "none":
                ...
                w = Weather.from_showdown_message(weather)
                if w not in self._weather:
                    self._weather = {
                        Weather.from_showdown_message(weather): deepcopy(self.turn)
                    }
            ...
            # self.get_pokemon(pokemon)._public_item = to_id_str(item)
            ...
            try:
                ...
                self.logger.warning("Skipped raw parse exception")
