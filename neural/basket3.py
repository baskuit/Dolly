...
class PolicyHead(torch.nn.Module):
    def __init__(
        ...
        self.move_linear = torch.nn.Linear(2**4, tower_dim)
        self.switch_linear = torch.nn.Linear(2**6, tower_dim)
        ...
        self.norm = torch.nn.LayerNorm((9, tower_dim))

    def forward(self, active_moves, latent_words):
        latent_our_bench = latent_words[1]
        tower_in = [
            self.move_linear(active_moves),
            self.switch_linear(latent_our_bench),
        ]
        tower_out = self.tower2(self.tower1(tower_in, latent_words))
        pre_logits = self.logit_linear_1(torch.cat(tower_out, dim=-2))
...
class BasketNet3(nn.Module):
    ...
    @torch.no_grad()
    def step_weights(self, other: "BasketNet3", lr: float):
